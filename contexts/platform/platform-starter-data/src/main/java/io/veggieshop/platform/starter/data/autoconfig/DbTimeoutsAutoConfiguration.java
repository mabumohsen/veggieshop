...
@ConditionalOnProperty(prefix = "veggieshop.db", name = "enabled", havingValue = "true", matchIfMissing = true)
public class DbTimeoutsAutoConfiguration {
    ...
    public BeanPostProcessor hikariTimeoutsCustomizer(DbTuningProperties props) {
        return new BeanPostProcessor() {
            ...
            public Object postProcessBeforeInitialization(Object bean, String beanName) {
                ...
                // ----- Pool sizing -----
                int maxPool = props.getMaxPoolSize() > 0
                ...
                if (isPg && props.isApplyPgTuning()) {
                    ...
                    if (props.getPgSocketTimeoutSeconds() > 0) {
                        ds.addDataSourceProperty("socketTimeout", String.valueOf(props.getPgSocketTimeoutSeconds()));
                    }
                ...
                log.info("HikariDataSource[{}]: maxPoolSize={}, connTimeout={}, validationTimeout={}, idleTimeout={}, maxLifetime={}{}",
                        beanName,
                        ...
                        isPg && props.isApplyPgTuning()
                                ? String.format(", pg[statement_timeout=%s, idle_in_tx_session_timeout=%s, socketTimeout=%ds]",
            ...
            private void safeSetMaxPoolSize(HikariDataSource ds, int size) {
                try {
                    ...
                    log.warn("Adjusted Hikari maximumPoolSize from {} to {} to satisfy minimumIdle={}",
                            size, fallback, ds.getMinimumIdle());
                }
            }
            ...
            private String buildPgInitSql(DbTuningProperties p) {
                ...
                if (p.getPgStatementTimeout().toMillis() > 0) {
                    sb.append("SET statement_timeout = ").append(p.getPgStatementTimeout().toMillis()).append("; ");
                }
                if (p.getPgIdleInTxSessionTimeout().toMillis() > 0) {
                    sb.append("SET idle_in_transaction_session_timeout = ")
                            .append(p.getPgIdleInTxSessionTimeout().toMillis()).append("; ");
                }
                return sb.toString().trim();
            }

            private String human(Duration d) {
                if (d == null) return "PT0S";
                long ms = d.toMillis();
    ...
    public TransactionManagerCustomizer<DataSourceTransactionManager> jdbcTxManagerCustomizer(DbTuningProperties props) {
        return (DataSourceTransactionManager tm) -> applyTxDefaults("JDBC", tm, props);
    }

    @Bean
    @ConditionalOnMissingBean(name = "jpaTxManagerCustomizer")
    public TransactionManagerCustomizer<JpaTransactionManager> jpaTxManagerCustomizer(DbTuningProperties props) {
        return (JpaTransactionManager tm) -> applyTxDefaults("JPA", tm, props);
    }

    private void applyTxDefaults(String kind, PlatformTransactionManager tm, DbTuningProperties props) {
        ...
        log.info("Applied {} transaction defaults: timeout={}s, rollbackOnCommitFailure={}, enforceReadOnly={}",
                kind, timeoutSec, props.isRollbackOnCommitFailure(), props.isEnforceReadOnly());
    }
    ...
    public static class DbTuningProperties {
        ...
        /**
         * Compute maximumPoolSize if not explicitly set: 4 * vCPU bounded by [poolSizeMin..poolSizeMax].
         */
