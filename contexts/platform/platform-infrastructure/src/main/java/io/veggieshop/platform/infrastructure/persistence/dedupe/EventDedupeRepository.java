 ...
 *
 * <p>Repository for the consumer de-duplication store (PostgreSQL 15+).
 * Aligns with PRD v2.0 exactly-once effects at the consumer layer:
 *   - Natural composite key: (tenant_id, event_id, event_version)
 *   - TTL-based retention (>= 7d) via expires_at
 *   - No PII; operational metadata for observability
 *
 * <p>Recommended usage pattern:
 *   1) In a consumer, attempt {@link #markProcessedIfAbsent(String, UUID, long, Instant, Duration, String, String, String, String)}.
 *   2) If returns true → proceed with side effects (or vice versa when side effects are idempotent on their own).
 *   3) If false → duplicate delivery; skip or route to idempotent path.
 *
 * <p>Notes:
 *   - Uses PostgreSQL's ON CONFLICT to guarantee atomic "insert if absent".
 *   - Batch TTL cleanup is exposed via {@link #deleteExpiredBefore(Instant, int)}.
 *   - Keep the call inside a transaction that also wraps your side-effects when possible.
 */
@Repository
public interface EventDedupeRepository extends JpaRepository<EventDedupeEntity, EventDedupeEntity.EventDedupeKey> {
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    int insertIgnoreConflict(@Param("tenantId") String tenantId,
     ...
     */
    @Transactional
    default boolean markProcessedIfAbsent(String tenantId,
                                          UUID eventId,
                                          long eventVersion,
                                          Instant now,
                                          Duration ttl,
        ...
        final int inserted = insertIgnoreConflict(
                tenantId, eventId, eventVersion, now, now.plus(ttl),
                consumer, eventFamily, traceId, payloadFingerprint
        );
        return inserted == 1;
    }
    ...
    default boolean markProcessedIfAbsent(String tenantId,
        ...
        return markProcessedIfAbsent(
                tenantId, eventId, eventVersion, now, EventDedupeEntity.DEFAULT_TTL,
                consumer, eventFamily, traceId, payloadFingerprint
        );
    }
    ...
    /**
     * Extend TTL (expires_at) only if the new value is further in the future.
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    int extendTtl(@Param("tenantId") String tenantId,
                  @Param("eventId") UUID eventId,
                  @Param("eventVersion") long eventVersion,
                  @Param("newExpiresAt") Instant newExpiresAt);

    /**
     * Delete expired rows in small batches for predictable vacuum/IO behavior.
     * Returns number of rows deleted in this pass.
     *
     * Uses ctid to avoid large DELETE w/o LIMIT. Repeat until returns 0.
     */
    @Modifying(flushAutomatically = false, clearAutomatically = false)
    @Transactional
    @Query(value = """
    ...
    @Query(value = "select count(*) from event_dedupe where event_family = :family", nativeQuery = true)
