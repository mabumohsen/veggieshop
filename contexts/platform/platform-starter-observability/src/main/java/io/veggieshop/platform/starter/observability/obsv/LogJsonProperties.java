...
public class LogJsonProperties {
  ...
  /**
   * Fail-closed posture: deny the logging event when a sensitive pattern is detected. If set to
   * false, the guard will allow the event; masking should then be applied by encoders.
   */
  ...
  public List<String> getExtraBlockedPatterns() {
    return List.copyOf(extraBlockedPatterns);
  }

  /** Defensive copy to avoid storing external mutable list directly. */
  public void setExtraBlockedPatterns(List<String> extraBlockedPatterns) {
    this.extraBlockedPatterns =
        (extraBlockedPatterns == null) ? new ArrayList<>() : new ArrayList<>(extraBlockedPatterns);
  }
}
