 ...
 * SamplingConfig
 *
 * <p>Optional rule-based head sampler for VeggieShop services. This complements tail-based sampling
 * at the collector by providing targeted head sampling during incidents or for critical paths,
 * while respecting parent sampling decisions.
 *
 * <p>PRD alignment: - Tail-based sampling at collector; dynamic head sampling (incident=100%,
 * targeted paths) (PRD §17) - No PII propagation; force-sample via explicit baggage allow-list only
 * - Servlet + Virtual Threads friendly (PRD §6)
 *
 * <p>Activation: - Enabled only if {@code veggieshop.obsv.sampling.rules-enabled=true}. -
...
public class SamplingConfig {
  ...
  @ConditionalOnProperty(
      prefix = "veggieshop.obsv.sampling",
      name = "rules-enabled",
      havingValue = "true")
  ...
  static final class RuleBasedSampler implements Sampler {
    ...
    private static final AttributeKey<String> MSG_SYSTEM =
        AttributeKey.stringKey("messaging.system");
    private static final AttributeKey<String> MSG_OP =
    ...
    RuleBasedSampler(SamplingProperties props) {
      ...
      this.parentBasedRatio =
          Sampler.parentBased(Sampler.traceIdRatioBased(props.getHeadSampleRatio()));
    ...
    public SamplingResult shouldSample(
      ...
      String route =
          firstNonNull(
              attributes.get(HTTP_ROUTE), attributes.get(HTTP_TARGET), attributes.get(URL_PATH));
      ...
      if (spanKind == SpanKind.CONSUMER && props.isSampleKafkaConsumers()) {
        ...
        if ("kafka".equals(system)
            && (op == null || "process".equals(op) || "receive".equals(op))) {
          return alwaysOn.shouldSample(
              parentContext, traceId, name, spanKind, attributes, parentLinks);
        }
      }
      ...
      return parentBasedRatio.shouldSample(
          parentContext, traceId, name, spanKind, attributes, parentLinks);
    }
  ...
  public static class SamplingProperties {
    ...
    /**
     * Incident mode: when true, sample 100% to maximize observability during incidents. Prefer
     * enabling this via configuration/feature flag during on-call events.
     */
    private boolean incidentMode = false;

    /**
     * Default head sampling ratio when none of the rules match (0.0..1.0). ParentBased +
     * traceIdRatioBased(headSampleRatio).
     */
    private double headSampleRatio = 0.10d;

    /**
     * Force-sample via baggage keys (case-insensitive values: 1/true/yes). These baggage keys must
     * be allow-listed at propagation level as well.
     */
    private List<String> forceBaggageKeys = List.of("forceSample");

    /** Drop spans whose names match any of these regex patterns (noise). */
    private List<String> dropSpanNamePatterns =
        List.of(
    ...
    private List<String> priorityRoutePatterns =
        List.of("^/v1/checkout.*", "^/v1/orders.*", "^/v1/payments.*");
    ...
    public boolean isIncidentMode() {
      return incidentMode;
    }

    public void setIncidentMode(boolean incidentMode) {
      this.incidentMode = incidentMode;
    }

    public double getHeadSampleRatio() {
      return headSampleRatio;
    }

    public void setHeadSampleRatio(double headSampleRatio) {
      this.headSampleRatio = headSampleRatio;
    }

    public List<String> getForceBaggageKeys() {
      return java.util.Collections.unmodifiableList(forceBaggageKeys);
    }

    public void setForceBaggageKeys(List<String> forceBaggageKeys) {
      this.forceBaggageKeys =
          (forceBaggageKeys == null) ? new ArrayList<>() : new ArrayList<>(forceBaggageKeys);
    }

    public List<String> getDropSpanNamePatterns() {
      return java.util.Collections.unmodifiableList(dropSpanNamePatterns);
    }

    /**
     * Replaces the list of span-name regex patterns that should be dropped.
     *
     * <p>The given list is defensively copied; {@code null} results in an empty list.
     *
     * @param dropSpanNamePatterns regex patterns for span names to drop
     */
    public void setDropSpanNamePatterns(List<String> dropSpanNamePatterns) {
      this.dropSpanNamePatterns =
          (dropSpanNamePatterns == null)
              ? new ArrayList<>()
              : new ArrayList<>(dropSpanNamePatterns);
    }

    public List<String> getPriorityRoutePatterns() {
      return java.util.Collections.unmodifiableList(priorityRoutePatterns);
    }

    /**
     * Replaces the list of priority HTTP route regex patterns that are always sampled.
     *
     * <p>The given list is defensively copied; {@code null} results in an empty list.
     *
     * @param priorityRoutePatterns regex patterns for priority HTTP routes
     */
    public void setPriorityRoutePatterns(List<String> priorityRoutePatterns) {
      this.priorityRoutePatterns =
          (priorityRoutePatterns == null)
              ? new ArrayList<>()
              : new ArrayList<>(priorityRoutePatterns);
    }

    public boolean isSampleKafkaConsumers() {
      return sampleKafkaConsumers;
    }

    public void setSampleKafkaConsumers(boolean sampleKafkaConsumers) {
      this.sampleKafkaConsumers = sampleKafkaConsumers;
    }
  }
}
