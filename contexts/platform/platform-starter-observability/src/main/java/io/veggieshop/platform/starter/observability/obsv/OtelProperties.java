 ...
 *
 * <p>Prefix: {@code veggieshop.otel}
 *
 * <h3>Design notes</h3>
 *
 * <ul>
 *   <li>Supports both OTLP gRPC and HTTP/Protobuf with sensible defaults.
 *   <li>Signal-specific endpoints (traces/metrics/logs) may be overridden individually. If not set,
 *       they are derived from {@link #endpoint} + protocol.
 *   <li>Batching and timeouts expose OTel SDK defaults with safe enterprise tweaks.
 *   <li>Sampler ratio is ParentBased(traceIdRatioBased) in {@link OtelAutoConfig}.
 *   <li>Baggage allow-list prevents propagating PII by default.
 * </ul>
 */
@Validated
...
public class OtelProperties {
   ...
   *
   * <ul>
   *   <li>{@code GRPC}: default port 4317, endpoint is host:port without path.
   *   <li>{@code HTTP}: default port 4318, endpoints include {@code /v1/{signal}} paths.
   * </ul>
   */
  @NotNull private Protocol protocol = Protocol.GRPC;

  /**
   * Base endpoint applied to all signals when a per-signal endpoint is not provided.
   *
   * <p>For GRPC, example: {@code http://localhost:4317}.<br>
  ...
  /**
   * Exporter timeout (applies to traces/metrics/logs exporters). Default: 10s (OTel defaults are
   * typically 10s).
   */
  @NotNull private Duration exporterTimeout = Duration.ofSeconds(10);

  /** Batch schedule delay for spans/logs processing. Default: 5s (OTel default). */
  @NotNull private Duration batchScheduleDelay = Duration.ofSeconds(5);

  /** Max queue size for batch processors. Default: 2048 (OTel default). */
  @Min(1)
  private int batchMaxQueue = 2048;

  /** Max export batch size for batch processors. Default: 512 (OTel default). */
  @Min(1)
  private int batchMaxExportBatchSize = 512;

  /** Periodic metric reader export interval. Default: 60s (balanced overhead vs. freshness). */
  @NotNull private Duration metricReaderInterval = Duration.ofSeconds(60);

  /**
   * ParentBased + ratio sampler ratio (0.0..1.0). Default: 0.05 (5%) — conservative head sampling;
  ...
  /**
   * Allowed baggage keys to propagate (case-insensitive). Defaults align with PRD: no PII
   * propagation.
   */
  @NotNull
  private List<String> baggageAllowedKeys =
      new ArrayList<>(List.of("cartId", "orderId", "tenantId"));
  ...
  /**
   * Returns the effective traces endpoint based on this precedence.
   *
   * <pre>
  ...
  /**
   * Returns the effective metrics endpoint using the same precedence as {@link
   * #getTracesEndpoint()}.
   */
  public String getMetricsEndpoint() {
    ...
    return switch (protocol) {
      case GRPC -> nonEmptyOrDefault(endpoint, DEFAULT_GRPC_BASE);
      case HTTP ->
          ensureHttpPath(nonEmptyOrDefault(endpoint, DEFAULT_HTTP_BASE), HTTP_METRICS_PATH);
    };
  }
  ...
  /**
   * Base endpoint for all signals (overridden by per-signal values if provided). For HTTP, do NOT
   * include {@code /v1/traces|metrics|logs}; we will append automatically.
   */
  ...
  public void setBatchScheduleDelay(Duration batchScheduleDelay) {
    this.batchScheduleDelay =
        (batchScheduleDelay == null) ? Duration.ofSeconds(5) : batchScheduleDelay;
  }
  ...
  public void setMetricReaderInterval(Duration metricReaderInterval) {
    this.metricReaderInterval =
        (metricReaderInterval == null) ? Duration.ofSeconds(60) : metricReaderInterval;
  }
  ...
  public void setTraceSampleRatio(double traceSampleRatio) {
    ...
    if (traceSampleRatio < 0.0) {
      traceSampleRatio = 0.0;
    }
    if (traceSampleRatio > 1.0) {
      traceSampleRatio = 1.0;
    }
    this.traceSampleRatio = traceSampleRatio;
  }

  /** Allowed baggage keys (unmodifiable view) to propagate downstream. */
  ...
  public void setBaggageAllowedKeys(List<String> baggageAllowedKeys) {
    this.baggageAllowedKeys =
        (baggageAllowedKeys == null) ? new ArrayList<>() : new ArrayList<>(baggageAllowedKeys);
  }

  // ======== Types ========

  /** OTLP transport protocol selection (gRPC vs HTTP/Protobuf). */
