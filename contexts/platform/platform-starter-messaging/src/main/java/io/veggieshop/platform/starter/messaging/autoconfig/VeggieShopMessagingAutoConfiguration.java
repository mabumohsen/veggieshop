...
public class VeggieShopMessagingAutoConfiguration {
    private static final Logger log = LoggerFactory.getLogger(VeggieShopMessagingAutoConfiguration.class);

    @Bean
    @ConditionalOnMissingBean
    DeadLetterPublishingRecoverer quarantineRecoverer(
            KafkaTemplate<Object,Object> kafkaTemplate,
            VeggieKafkaErrorProperties p) {
    ...
    DefaultErrorHandler kafkaConsumerErrorHandler(
        ...
        RetryListener rl = new RetryListener() {
            @Override public void failedDelivery(ConsumerRecord<?, ?> rec, Exception ex, int attempt) {
                log.warn("consumer_retry_failed topic={} partition={} offset={} attempt={} err={} root={}",
                        rec.topic(), rec.partition(), rec.offset(), attempt, ex.getClass().getName(),
                        ConsumerErrorHandling.rootCause(ex).getClass().getName());
            }
        };
        return ConsumerErrorHandling.buildDefaultErrorHandler(
                recoverer,
                p.getMaxRetries(), p.getInitialIntervalMs(), p.getMultiplier(), p.getMaxIntervalMs(),
                p.getJitterFraction(),
                p.isCommitRecovered(),
                rl
        );
    ...
    @Bean
    @ConditionalOnMissingBean
    ReliablePublisher reliablePublisher(KafkaTemplate<Object,Object> kafkaTemplate,
    ...
    @ConditionalOnMissingBean
    OutboxPublisher outboxPublisher(JdbcTemplate jdbc,
    ...
    @Bean
    @ConditionalOnMissingBean
    DedupeService dedupeService(JdbcTemplate jdbc,
