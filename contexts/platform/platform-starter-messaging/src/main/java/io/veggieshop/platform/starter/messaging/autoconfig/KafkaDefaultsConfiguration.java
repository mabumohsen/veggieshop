...
public class KafkaDefaultsConfiguration implements EnvironmentAware {
    ...
    public KafkaDefaultsConfiguration(KafkaProperties springKafka,
                                      VeggieKafkaProperties props,
    ...
    public KafkaTemplate<String, Object> kafkaTemplate(
        ...
        if (listener != null) tpl.setProducerListener(listener);
        return tpl;
    }
    ...
    public ConcurrentKafkaListenerContainerFactory<String, Message> protobufKafkaListenerContainerFactory(
        ...
        ConcurrentKafkaListenerContainerFactory<String, Message> f = new ConcurrentKafkaListenerContainerFactory<>();
        f.setConsumerFactory(cf);
        f.getContainerProperties().setObservationEnabled(true);
        f.getContainerProperties().setPollTimeout(props.getConsumer().getPollTimeout().toMillis());
        f.getContainerProperties().setAckMode(props.getConsumer().getAckMode());
    ...
    public DeadLetterPublishingRecoverer avroDltRecoverer(
        ...
        return new DeadLetterPublishingRecoverer(
                avroTemplate,
                (record, ex) -> new TopicPartition(record.topic() + props.getDltSuffix(), record.partition()));
    }
    ...
    private Map<String, Object> baseProducerProps() {
        ...
        cfg.put(ProducerConfig.DELIVERY_TIMEOUT_MS_CONFIG, (int) props.getProducer().getDeliveryTimeout().toMillis());
        cfg.put(ProducerConfig.REQUEST_TIMEOUT_MS_CONFIG, (int) props.getProducer().getRequestTimeout().toMillis());
    ...
    private Map<String, Object> baseConsumerProps() {
        ...
        cfg.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, (int) props.getConsumer().getMaxPollInterval().toMillis());
        cfg.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, (int) props.getConsumer().getSessionTimeout().toMillis());
        cfg.put(ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG, (int) props.getConsumer().getHeartbeatInterval().toMillis());
        cfg.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, (int) props.getConsumer().getFetchMaxWait().toMillis());
    ...
    private void schemaRegistry(Map<String, Object> cfg) {
        if (StringUtils.hasText(props.getSchemaRegistry().getUrl())) {
            cfg.put(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, props.getSchemaRegistry().getUrl());
            cfg.put(AbstractKafkaSchemaSerDeConfig.AUTO_REGISTER_SCHEMAS, props.getSchemaRegistry().isAutoRegisterSchemas());
            cfg.put(AbstractKafkaSchemaSerDeConfig.USE_LATEST_VERSION, props.getSchemaRegistry().isUseLatestVersion());
            if (StringUtils.hasText(props.getSchemaRegistry().getBasicAuthUserInfo())) {
                ...
                cfg.put(AbstractKafkaSchemaSerDeConfig.USER_INFO_CONFIG, props.getSchemaRegistry().getBasicAuthUserInfo());
            }
        }
    }

    private void maybeSecurity(Map<String, Object> cfg) {
        if (props.getSecurity() == null) return;
    ...
    private List<String> bootstrapServers() {
        ...
        if (bs != null && !bs.isEmpty()) return bs;
        return springKafka.getBootstrapServers();
    }

    private String buildClientId() {
        String app = environment != null ? environment.getProperty("spring.application.name", "veggieshop") : "veggieshop";
        String host;
        try {
