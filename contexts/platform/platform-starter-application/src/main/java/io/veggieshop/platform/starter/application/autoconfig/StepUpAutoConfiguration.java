...
import io.veggieshop.platform.application.security.StepUpService.ApprovalBroker;
import io.veggieshop.platform.application.security.StepUpService.ApprovalRequest;
import io.veggieshop.platform.application.security.StepUpService.ApprovalStatus;
import io.veggieshop.platform.application.security.StepUpService.AuditSink;
import io.veggieshop.platform.application.security.StepUpService.ElevationStore;
import io.veggieshop.platform.application.security.StepUpService.MfaProvider;
import io.veggieshop.platform.application.security.StepUpService.StepUpChallenge;
import io.veggieshop.platform.application.security.StepUpService.StepUpTicket;
...
/**
 * Auto-configuration for Step-Up authentication: wires defaults for development/local setups and
 * exposes the primary {@link StepUpService} when the application does not provide beans.
 */
@AutoConfiguration
...
public class StepUpAutoConfiguration {
  ...
  // ====================== Development/Local defaults (optional) ======================
  // These beans are created only if the application does not provide its own implementations.

  @Bean
  @ConditionalOnMissingBean(MfaProvider.class)
  @ConditionalOnProperty(
      prefix = "veggieshop.stepup.dev",
      name = "mfa-provider",
  ...
  AuditSink loggingAuditSink() {
    return event -> {
      // Keep audit payload minimal in the starter; applications should provide a proper sink.
      System.out.println(
          "[AUDIT] type="
              + event.type()
              + " tenant="
  ...
  // ======= Simple in-memory implementations for development purposes =======
  ...
  static final class InMemoryMfaProvider implements MfaProvider {
    ...
    @Override
    public Optional<StepUpChallenge> findActiveChallengeByKey(
    ...
    @Override
    public Optional<StepUpChallenge> findChallengeById(
        String tenantId, String userId, String challengeId) {
      StepUpChallenge c = byId.get(challengeId);
      if (c == null) {
        return Optional.empty();
      }
      if (!tenantId.equals(c.tenantId()) || !userId.equals(c.userId())) {
        return Optional.empty();
      }
    ...
    public boolean verifyChallenge(StepUpChallenge challenge, String otpOrProof) {
      // For dev only: treat any value of length >= 6 as valid.
      return otpOrProof != null && otpOrProof.length() >= 6;
    }

    @Override
    public void closeChallenge(String challengeId) {
      byId.remove(challengeId);
    }

    private static String key(String t, String u, String k) {
      return t + "|" + u + "|" + k;
    }
  }
  ...
  static final class InMemoryElevationStore implements ElevationStore {
    ...
    @Override
    public void grant(StepUpTicket t) {
      tickets.put(t.token(), t);
    }
    ...
    public Optional<StepUpTicket> findActive(String tenantId, String userId, Instant now) {
      return tickets.values().stream()
          .filter(
              t -> t.tenantId().equals(tenantId) && t.userId().equals(userId) && t.isActive(now))
          .findFirst();
    }
  ...
  static final class InMemoryApprovalBroker implements ApprovalBroker {
    ...
    @Override
    public Optional<ApprovalRequest> findOpenByKey(
        String tenantId, String requesterUserId, String idempotencyKey) {
      String id = openByKey.get(key(tenantId, requesterUserId, idempotencyKey));
      if (id == null) {
        return Optional.empty();
      }
      ApprovalRequest r = store.get(id);
      return (r != null && r.status() == ApprovalStatus.PENDING)
          ? Optional.of(r)
          : Optional.empty();
    }
    ...
    @Override
    public void update(ApprovalRequest updated) {
    ...
    public void expire(String approvalId) {
      store.computeIfPresent(
          ...
          (id, cur) ->
              cur.withStatus(
                  ApprovalStatus.EXPIRED, cur.decidedBy(), cur.decisionComment(), Instant.now()));
    }
