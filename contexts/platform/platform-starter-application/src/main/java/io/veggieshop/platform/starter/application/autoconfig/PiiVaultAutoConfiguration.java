...
/**
 * Auto-configuration for PII Vault integrations, ABAC policy engine, consistency guards, and
 * tracing primitives used by applications that depend on PII Vault.
 */
@AutoConfiguration
@AutoConfigureAfter(
    name = "io.veggieshop.platform.starter.data.autoconfig.VeggiePiiVaultAutoConfiguration")
public class PiiVaultAutoConfiguration {
  ...
  AbacPolicyEngine abacPolicyEngine(
      MeterRegistry metrics, Clock clock, ObjectProvider<AbacDefaults> defaultsProvider) {
  ...
  ReadYourWritesGuard readYourWritesGuard(
      ConsistencyService consistency, ConsistencyProperties props, Clock clock) {
  ...
  PiiVaultClient piiVaultClient(
      PiiVaultClient.PiiVaultPort port,
