 ...
 * PiiLogGuardFilter
 *
 * Enterprise-grade, zero-surprise PII guard for logs/metrics/traces.
 *
 * What it does (without changing request semantics):
 *  - Establishes a strict “sensitive-key” deny-list for headers, parameters, and MDC keys.
 *  - Publishes sanitized views of headers/parameters on request attributes so downstream (Problem handler,
 *    access logs, OTEL enrichers) can log safely.
 *  - Scrubs MDC of keys that look sensitive (keeps only an allow-list like traceId/spanId/tenantId/requestId/correlationId).
 *
 * Notes:
 *  - Does not buffer/read body; does not alter response.
 *  - Bean is created by platform-starter-web (auto-config). No Spring stereotype here.
 */
...
public final class PiiLogGuardFilter extends OncePerRequestFilter {
    ...
    /** Request attributes exposing sanitized, log-safe views. */
    public static final String ATTR_SANITIZED_HEADERS    = PiiLogGuardFilter.class.getName() + ".SANITIZED_HEADERS";
    public static final String ATTR_SANITIZED_PARAMETERS = PiiLogGuardFilter.class.getName() + ".SANITIZED_PARAMETERS";

    /** Canonical non-PII platform header we allow to appear in clear form. */
    public static final String HEADER_TENANT_ID = "X-Tenant-Id";

    /** Allowed MDC keys (non-PII, per PRD v2.0). */
    private static final Set<String> ALLOWED_MDC_KEYS = Set.of(
    ...
    private static final Set<String> SENSITIVE_BASE = Set.of(
            // Auth / Secrets
            "authorization","proxy-authorization","x-api-key","api-key","apikey","api-key","secret","client-secret",
            "access-token","id-token","refresh-token","token","session","sessionid","xsrf","csrf",
            // Credentials
            "password","pass","pwd","pin","otp",
            // Payments / PCI
            "card","pan","cvv","cvc","expiry","iban","swift","bank","account-number",
            // Identity / Contact (PII)
            "ssn","national-id","nationalid","nin","passport","email","e-mail","phone","mobile","msisdn",
            "address","street","zipcode","postal","dob","birth","firstname","lastname","fullname","name",
            // Cookies
            "cookie","set-cookie",
            // Misc sensitive
            "geo","location","device-id","deviceid","license","private-key","privatekey"
    );

    // ---------------- Configuration (immutable) ----------------

    private final int payloadMaxChars;
    /** case-insensitive header denylist (stored lowercase) */
    ...
    /**
     * Default constructor (sane defaults). Prefer using the parameterized constructor via auto-config.
     */
    public PiiLogGuardFilter() {
        this(2048,
                new LinkedHashSet<>(List.of("authorization","cookie","set-cookie","x-api-key")),
                new ArrayList<>(List.of("\\b[0-9]{13,19}\\b","[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}"))
        );
    }
     ...
     *
     * @param payloadMaxChars  maximum logged body chars after redaction
     * @param headerDenylist   case-insensitive header names to redact
     * @param redactPatterns   regex patterns for payload redaction
     */
    public PiiLogGuardFilter(int payloadMaxChars, Set<String> headerDenylist, List<String> redactPatterns) {
    ...
    private boolean isSensitiveKey(String key) {
        ...
        if (sensitiveTokens.contains(norm)) return true;
        // Heuristic match (e.g., x-client-secret, user-email, etc.)
        for (String token : sensitiveTokens) {
    ...
    private static Set<String> toLowerLinkedSet(Set<String> in) {
        if (in == null || in.isEmpty()) return new LinkedHashSet<>();
