...
/**
 * Reliable (instrumented) Kafka publisher with retries, exponential backoff + jitter, Micrometer
 * metrics, and OpenTelemetry propagation.
 *
 * <p>Features:
 *
 * <ul>
 *   <li>Injects a standard envelope (tenant/trace/schema/version) and propagates W3C Trace Context.
 *   <li>Retry with exponential backoff and jitter to reduce thundering-herd effects.
 *   <li>Micrometer counters/timers and OpenTelemetry spans per send.
 *   <li>Sync API ({@link #publish}) and async API ({@link #publishAsync}).
 * </ul>
 */
public class ReliablePublisher {
  ...
  /** Log format constants to keep source lines short. */
  private static final String LOG_OK_FMT =
      "kafka_publish_ok topic={} partition={} offset={} keyPresent={} tenant={} "
          + "eventId={} attempt={}";

  private static final String LOG_FAIL_FMT =
      "kafka_publish_fail topic={} keyPresent={} tenant={} eventId={} attempt={} "
          + "retryable={} errorClass={}";

  /** W3C Trace Context injector to Kafka headers via OpenTelemetry. */
  ...
  /**
   * Constructs with pragmatic defaults.
   *
   * <ul>
   *   <li>maxAttempts = 3
   *   <li>initialBackoff = 250ms
  ...
  public ReliablePublisher(
      KafkaTemplate<Object, Object> kafkaTemplate, MeterRegistry meterRegistry) {
    this(
        kafkaTemplate,
        meterRegistry,
  ...
  /** Publishes synchronously (blocks until ack or timeout). */
  public SendResult<Object, Object> publish(
      String topic, Object key, Object value, PublisherOptions options) {
    try {
  ...
  /** Publishes asynchronously and completes the future upon success/failure. */
  ...
  private CompletableFuture<SendResult<Object, Object>> publishInternal(
      String topic, Object key, Object value, PublisherOptions options, boolean logBlocking) {
    ...
    // Standard envelope (tenant/trace/schema/version).
    String currentTraceId = currentTraceId();
    Headers.attachEnvelope(
        headers,
        coalesce(
            options.tenantId(),
            Headers.getAsString(options.extraHeaders(), Headers.Keys.TENANT_ID).orElse(null)),
        coalesce(
            currentTraceId,
            Headers.getAsString(options.extraHeaders(), Headers.Keys.TRACE_ID).orElse(null)),
        options.schemaFingerprint(),
        options.entityVersion());

    // Inject W3C Trace Context from the current OTel context.
    GlobalOpenTelemetry.getPropagators()
        .getTextMapPropagator()
        .inject(Context.current(), headers, KAFKA_HEADER_SETTER);

    // Stable event-id for downstream idempotency.
    String eventId = coalesce(options.eventId(), UUID.randomUUID().toString());
    Headers.put(headers, Headers.Keys.EVENT_ID, eventId);
    ...
    Timer timer =
        Timer.builder("messaging.kafka.publish.latency")
    ...
    while (true) {
      ...
      Span span = tracer.spanBuilder("kafka.publish").setSpanKind(SpanKind.PRODUCER).startSpan();
      try {
        ...
        span.setAttribute(
            "messaging.kafka.partition", record.partition() == null ? -1 : record.partition());
        span.setAttribute(
            "veggieshop.tenant_id",
        ...
        final SendResult<Object, Object> result =
            cf.get(sendTimeout.toMillis(), TimeUnit.MILLISECONDS);
        final var md = result.getRecordMetadata();
        ...
        handleFailure(ie, attempt, topic, key, headers, eventId, tags, span, terminal, logBlocking);
      } catch (ExecutionException ee) {
        Throwable root = (ee.getCause() != null) ? ee.getCause() : ee;
        handleFailure(
            root, attempt, topic, key, headers, eventId, tags, span, terminal, logBlocking);
      } catch (TimeoutException te) {
        handleFailure(te, attempt, topic, key, headers, eventId, tags, span, terminal, logBlocking);
      } catch (KafkaException ke) {
        handleFailure(ke, attempt, topic, key, headers, eventId, tags, span, terminal, logBlocking);
      } catch (RuntimeException re) {
        handleFailure(re, attempt, topic, key, headers, eventId, tags, span, terminal, logBlocking);
      } finally {
        span.end();
      }
    }
  ...
  private void handleFailure(
      ...
      Tags tags,
      Span span,
      CompletableFuture<SendResult<Object, Object>> terminal,
      boolean logBlocking) {
    ...
    meterRegistry
        .counter(
            "messaging.kafka.publish.failure",
            Tags.of("topic", topic, "retryable", Boolean.toString(retryable)))
        .increment();
    ...
    if (logBlocking && log.isDebugEnabled()) {
      log.debug(
          "kafka_publish_retry topic={} attempt={} backoffMs={}", topic, attempt, sleep.toMillis());
    }
    sleepQuietly(sleep);
  }
  ...
  private boolean isRetryable(Throwable t) {
    return (t instanceof RetriableException) || (t instanceof KafkaException);
  }

  private Duration nextBackoff(int attempt) {
    if (attempt <= 1) {
      return withJitter(initialBackoff);
    }
  ...
  /** Immutable per-call options for publishing. */
  public record PublisherOptions(
    ...
    /** Fluent builder for {@link PublisherOptions}. */
    public static final class Builder {
      ...
      /**
       * Adds extra headers from a map with UTF-8 string values.
       *
       * <p>Null-safe: if {@code map} is null, an empty carrier is used.
       *
       * @param map header key/value pairs to add
       * @return this builder
       */
      public Builder extraHeaders(Map<String, String> map) {
        RecordHeaders h = new RecordHeaders();
        if (map != null) {
          map.forEach((k, v2) -> Headers.put(h, k, v2));
        }
        this.extraHeaders = h;
        return this;
      }
      ...
      /**
       * Adds extra headers from an existing Kafka headers carrier.
       *
       * @param h existing headers to attach (used as-is; not copied)
       * @return this builder
       */
      ...
      /**
       * Builds an immutable {@link PublisherOptions} snapshot.
       *
  ...
  /** Exception thrown when publishing fails after all retries. */
