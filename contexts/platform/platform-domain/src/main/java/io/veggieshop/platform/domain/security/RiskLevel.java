 ...
 * RiskLevel
 *
 * <p>Enterprise-grade risk taxonomy used across authorization, step-up MFA, and operational controls.
 * The scale is intentionally small, ordered, and stable to keep policies and audits predictable.</p>
 *
 * <h2>Levels (ascending severity)</h2>
 * <ul>
 *   <li><b>LOW</b> – Routine operations on non-sensitive resources.</li>
 *   <li><b>MEDIUM</b> – Important actions or internal data; extra scrutiny.</li>
 *   <li><b>HIGH</b> – Sensitive operations or data classes (PII/PHI); step-up MFA is typically required.</li>
 *   <li><b>CRITICAL</b> – Highly sensitive or dangerous actions (pricing overrides, refunds, admin changes);
 *       two-person approval and strict controls are expected.</li>
 * </ul>
 *
 * <h2>Design notes</h2>
 * <ul>
 *   <li>Implements common helpers (escalate/deescalate, max, fromScore) for consistent policy decisions.</li>
 *   <li>Provides {@link #recommendedControls()} as a sane default aligned with the PRD (MFA/2-person approval).</li>
 *   <li>No framework dependencies; safe for use in domain and application layers.</li>
 * </ul>
 */
public enum RiskLevel {
    ...
    /** @return true if this level is >= other. */
    ...
    public RiskLevel clamp(RiskLevel min, RiskLevel max) {
        ...
        if (this.ordinal() < min.ordinal()) return min;
        if (this.ordinal() > max.ordinal()) return max;
        return this;
    }
    ...
    public static RiskLevel max(RiskLevel... levels) {
        if (levels == null || levels.length == 0) return LOW;
        RiskLevel out = LOW;
        for (RiskLevel rl : levels) {
            if (rl != null && rl.ordinal() > out.ordinal()) out = rl;
        }
        return out;
    }
    ...
    /** Lenient, case-insensitive parse. Returns {@link #LOW} if input is null/blank. */
    public static RiskLevel parseOrDefault(String value, RiskLevel defaultLevel) {
        if (value == null || value.isBlank()) return defaultLevel == null ? LOW : defaultLevel;
    ...
    /**
     * Recommended controls for the given level.
     * These are defaults aligned with the PRD; policy engines can add stricter gates.
     */
    ...
    /**
     * Control hints consumed by higher layers (policy engine, interceptors, UI).
     * These are <b>signals</b>, not automatic behaviors in the domain layer.
     */
    public enum Control {
