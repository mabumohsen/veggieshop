 ...
 * AuditHash
 *
 * <p>Enterprise-grade, framework-agnostic content hash used for audit chaining
 * ({@code prevHash/hash}) as specified in VeggieShop PRD v2.0. This value object:
 * </p>
 *
 * <ul>
 *   <li>Uses a <strong>versioned domain separator</strong> to prevent cross-protocol collisions.</li>
 *   <li>Defaults to <strong>SHA-256</strong> (stable, FIPS-acceptable in many environments).</li>
 *   <li>Provides <strong>deterministic chaining</strong> over {@code prevHash || payload}.</li>
 *   <li>Exposes safe factory/parse methods and immutable, constant-time equality.</li>
 *   <li>String form: {@code sha256:<base64url_nopad>} (mirrors PRD header style, e.g. schema fingerprints).</li>
 * </ul>
 *
...
public final class AuditHash {
    ...
    public static AuditHash ofBase64Url(String base64Url) {
        try {
            return ofBytes(DEFAULT_ALGORITHM, B64_URL_DEC.decode(requireNonBlank(base64Url, "base64Url")));
    ...
    /**
     * Parse flexible textual form:
     * <ul>
     *   <li>{@code sha256:<base64url_nopad>}</li>
     *   <li>{@code sha256:<hex>}</li>
     * </ul>
     */
     ...
     * <p>If {@code prev} is {@code null}, this behaves like {@link #compute(byte[])}.</p>
     */
    public static AuditHash computeChained(AuditHash prev, byte[] payload) {
        ...
        if (prev != null) {
            ...
            if (prev.algorithm != DEFAULT_ALGORITHM) {
                throw new IllegalArgumentException("Algorithm mismatch in chain: expected " + DEFAULT_ALGORITHM.id());
            }
    ...
    /** Defensive copy of hash bytes. */
    public byte[] toByteArray() { return bytes.clone(); }

    /** Base64url (no padding) representation of the raw bytes. */
    public String toBase64Url() { return B64_URL_NOPAD_ENC.encodeToString(bytes); }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof AuditHash that)) return false;
        if (this.algorithm != that.algorithm) return false;
    ...
    private static byte[] validateAndCopy(Algorithm algorithm, byte[] in) {
        ...
        if (in.length != algorithm.lengthBytes()) {
            throw new IllegalArgumentException(
                    "Invalid hash length for " + algorithm.id() + ": expected " + algorithm.lengthBytes() + " bytes, got " + in.length);
        }
        return in.clone();
    }
    ...
    private static String requireNonBlank(String s, String what) {
        if (s == null || s.isBlank()) throw new IllegalArgumentException(what + " must not be blank");
        return s;
    }
    ...
    private static byte[] hexToBytes(String hex) {
        ...
        if (!isLikelyHex(h)) throw new IllegalArgumentException("Invalid hex string");
        int len = h.length();
