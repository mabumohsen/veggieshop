 ...
 * ProblemTypes
 *
 * <p>Central registry of RFC7807 <em>problem type</em> URIs for VeggieShop. These types are stable,
 * human-readable identifiers meant to be returned by HTTP and messaging layers to convey
 * machine-friendly error semantics.
 *
 * <h2>Principles</h2>
 *
 * <ul>
 *   <li><b>Stable URIs:</b> {@code https://problems.veggieshop.io/{slug}} with lowercase,
 *       kebab-case slugs.
 *   <li><b>PRD alignment:</b> covers multi-tenancy, idempotency, consistency tokens, concurrency,
 *       rate limiting, PCI/SCA, webhooks, schema/contracts, and quotas.
 *   <li><b>Framework-agnostic:</b> pure JDK 21; no Spring or servlet dependencies here.
 *   <li><b>Interoperability:</b> each type carries a sensible <i>default</i> HTTP status and a
 *       short title. Actual status codes are chosen by higher layers but should default to these.
 * </ul>
 *
 * <h3>Usage</h3>
 *
 ...
 * //   "type": t.uri(), "title": t.title(),
 * //   "status": t.defaultStatus(), "detail": "...", "instance": "..."
 * }</pre>
 */
public final class ProblemTypes {
  ...
  public static final ProblemType TENANT_REQUIRED = def("tenant-required", "Tenant Required", 400);

  /** 404 – Supplied tenant does not exist or is disabled. */
  public static final ProblemType TENANT_UNKNOWN = def("tenant-unknown", "Unknown Tenant", 404);

  /** 403 – Token tenant differs from request tenant. */
  public static final ProblemType TENANT_MISMATCH = def("tenant-mismatch", "Tenant Mismatch", 403);
  ...
  public static final ProblemType JWT_INVALID = def("jwt-invalid", "JWT Invalid", 401);
  ...
  public static final ProblemType ENDPOINT_SUNSET = def("endpoint-sunset", "Endpoint Sunset", 410);
  ...
  public static final ProblemType CONFLICT = def("conflict", "Conflict", 409);
  ...
  public static final ProblemType RATE_LIMITED = def("rate-limited", "Rate Limited", 429);

  /** 429 – Quota exceeded (e.g., storage/search/requests). */
  public static final ProblemType QUOTA_EXCEEDED = def("quota-exceeded", "Quota Exceeded", 429);
  ...
  public static final ProblemType INTERNAL_ERROR = def("internal-error", "Internal Error", 500);
  ...
  static {
    ...
    for (ProblemType t :
        new ProblemType[] {
          ...
          ENDPOINT_SUNSET,
          CONSISTENCY_PRECONDITION_FAILED,
  ...
  /**
   * Creates a {@link ProblemType} for a custom slug without registering it globally. Use for
   * domain-specific, one-off errors until promoted to a shared type.
   *
   * @param slug lower-kebab-case slug
   * @param title short title
   * @param defaultStatus default HTTP status
   * @return new {@link ProblemType}
   */
  ...
  /**
   * Finds a registered type by slug.
   *
   * @param slug the lower-kebab-case slug
   * @return optional containing the registered type, or empty if not found
   */
  ...
  /**
   * Returns an unmodifiable view of the registered types keyed by slug.
   *
   * @return map view of registry (slug -&gt; type)
   */
  ...
  /**
   * Composes the canonical type URI for a given valid slug.
   *
   * @param slug lower-kebab-case slug
   * @return absolute {@link URI} under {@link #BASE}
   */
  ...
  public record ProblemType(String slug, URI uri, String title, int defaultStatus) {
    ...
    /** Validates non-null record components at construction time. */
    public ProblemType {
     ...
     *
     * @return string like {@code validation-failed (400 -> https://...)}
     */
    @Override
