 ...
 * {@code UUID} primary key composed with {@code tenant_id}.</p>
 *
 * <h2>Design goals</h2>
 * <ul>
 *   <li><b>Strong typing:</b> wraps a {@link UUID}; avoids “naked string” keys across the codebase.</li>
 *   <li><b>Strict validation:</b> accepts only RFC&nbsp;4122 UUIDs (v1–v5), with flexible parsing of
 *       hyphenated and 32-hex formats; rejects malformed values early.</li>
 *   <li><b>Framework-agnostic:</b> no servlet/Spring/Kafka dependencies; suitable for domain layer.</li>
 *   <li><b>Ergonomics:</b> helpers to parse from header maps, generate secure random keys, and convert to bytes.</li>
 * </ul>
 *
 * <h3>Usage</h3>
 * <pre>{@code
...
public final class IdempotencyKey implements Comparable<IdempotencyKey>, Serializable {
    ...
    private static final List<String> HEADER_ALIASES = List.of(
            REQUEST_HEADER, "idempotency-key", "IDEMPOTENCY-KEY"
    );
    ...
    private static final Pattern UUID_HEX32 =
    ...
    private IdempotencyKey(UUID value) {
        ...
        if (value.variant() != 2 /* IETF RFC 4122 */) {
            throw new IllegalArgumentException("Idempotency key must be an RFC 4122 UUID (IETF variant).");
        }
        int v = value.version();
        if (v < 1 || v > 5) {
     ...
     * <ul>
     *   <li>Canonical UUID (hyphenated), e.g., {@code 123e4567-e89b-12d3-a456-426614174000}</li>
     *   <li>32 hex characters (no hyphens); will be normalized to canonical form</li>
     * </ul>
    ...
    public static IdempotencyKey parse(String raw) {
        ...
        if (s == null) throw new IllegalArgumentException("Idempotency key is required.");
        final UUID uuid;
    ...
    /**
     * Resolve from a case-insensitive headers map, checking common aliases.
     * Returns empty if absent or invalid.
     */
    ...
    @Override public int compareTo(IdempotencyKey o) {
    ...
    private static UUID uuidFromHex32(String hex32) {
        ...
        String canonical = h.substring(0, 8) + "-" +
                h.substring(8, 12) + "-" +
                h.substring(12, 16) + "-" +
                h.substring(16, 20) + "-" +
                h.substring(20);
