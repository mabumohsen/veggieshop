 ...
 * EntityVersion
 *
 * <p>A small, immutable value object that represents a monotonic entity version. Designed to be
 * framework-agnostic and safe across storage/transport layers.
 *
 * <p>Invariants: - value >= 0 (0 can be used to mean "unversioned"/"not yet persisted" if your
 * domain needs that) - strictly increasing across updates (enforced by callers using {@link
 * #next()})
 *
 * <p>Notes: - This type intentionally avoids HTTP concerns (ETag formatting/parsing). Keep
 * HTTP-specific logic in the web layer.
 */
public final class EntityVersion implements Comparable<EntityVersion>, Serializable {
  ...
  /**
   * Tries to parse a decimal string into an {@link EntityVersion}. Returns empty on
   * null/blank/invalid format or negative numbers.
   */
  ...
  /**
   * Indicates whether this instance is "unversioned".
   *
   * @return {@code true} if value == 0, otherwise {@code false}.
   */
  ...
  /** Convenience helper that checks if this version is before another. */
  public boolean isBefore(EntityVersion other) {
    Objects.requireNonNull(other, "other");
    return this.value < other.value;
  }

  /** Returns the max (greater-or-equal) of two versions. */
  ...
  /** Returns the min (less-or-equal) of two versions. */
