 ...
 * ConsistencyToken
 *
 * <p>A compact, signed token that carries a tenant-scoped "read-your-writes" watermark. Format
 * (compact string): CT1.&lt;kid&gt;.&lt;base64url(payload)&gt;.&lt;base64url(signature)&gt;.
 *
 * <p>- "CT1": version prefix (string literal). - kid: key id used by the signer for rotation. -
 * payload (JSON): {"t":"...","iat":1700000000000,"wm":1700000000420,"ver":15}. - signature: HMAC
 * over bytes("CT1." + kid + "." + base64url(payload)).
 *
 * <p>JSON field names are short on purpose: - t: tenant id (string) - iat: issued-at epoch millis
 * (long) - wm: watermark (epoch millis, monotonic per tenant) (long) - ver: optional entity version
 * (nullable).
 *
 * <p>This type is framework-agnostic and uses only Jackson for JSON.
 */
...
public final class ConsistencyToken implements Serializable {
  ...
  public static ConsistencyToken of(
      String tenant, long issuedAtMillis, long watermarkMillis, Long version) {
  ...
  /** Issued-at timestamp in epoch millis. */
  public long issuedAtMillis() {
    return issuedAtMillis;
  }

  /** Watermark timestamp in epoch millis. */
  public long watermarkMillis() {
    return watermarkMillis;
  }

  /** Optional embedded entity version. */
  public Optional<Long> version() {
    return Optional.ofNullable(version);
  }

  /**
   * Returns whether the token is expired with the given TTL.
   *
   * @param ttlMillis TTL in milliseconds; {@code <= 0} means "never expire".
   * @param clock time source to compare against
  ...
  /**
   * Parse and verify a compact token string.
   *
   * <p>Returns {@link Optional#empty()} if verification fails or the format is invalid.
   */
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ConsistencyToken that)) {
      return false;
    }
  ...
  public String toString() {
    ...
    return "ConsistencyToken{tenant='"
        + tenant
        + "', iat="
