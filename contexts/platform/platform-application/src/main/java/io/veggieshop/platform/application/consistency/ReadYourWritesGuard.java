 ...
 * ReadYourWritesGuard
 *
 * <p>Waits (briefly) before a READ to honor an If-Consistent-With token's watermark. This is a
 * cooperative "best effort": it avoids long blocking and returns false on timeout.
 */
public final class ReadYourWritesGuard {
  ...
  private static final Metrics NOOP_METRICS =
      new Metrics() {
        @Override
        ...
        @Override
        public void recordImmediateHit() {
          /* no-op */
        }
      };
  ...
  /** Creates a guard with explicit metrics sink. */
  public ReadYourWritesGuard(
      ConsistencyService service, ConsistencyProperties props, Clock clock, Metrics metrics) {
  ...
  /** Creates a guard that uses a no-op metrics sink. */
  public ReadYourWritesGuard(ConsistencyService service, ConsistencyProperties props, Clock clock) {
    this(service, props, clock, NOOP_METRICS);
  }

  /**
   * If the current request carries If-Consistent-With, wait until the tenant's watermark reaches
   * the requested value or until the configured timeout expires.
   *
  ...
  /**
   * Executes the supplied read only if {@link #awaitIfRequested()} succeeds.
   *
   * <p>Returns {@code Optional.empty()} when the guard times out, allowing callers to decide
   * whether to retry, fall back, أو إرسال 409/412 حسب البوليـسي.
   */
