...
public class HmacWebProperties {
  ...
  @NotBlank private String keyIdHeader = "X-HMAC-Key-Id";

  /** Header carrying the unix-epoch timestamp (seconds or millis per your filter). */
  @NotBlank private String timestampHeader = "X-HMAC-Timestamp";

  /** Header carrying the unique nonce. */
  @NotBlank private String nonceHeader = "X-HMAC-Nonce";

  /** Header carrying the computed signature. */
  @NotBlank private String signatureHeader = "X-HMAC-Signature";

  /** Accepted HMAC algorithms (e.g., HmacSHA256, HmacSHA512). First entry is used. */
  @NotNull private List<String> acceptedAlgorithms = new ArrayList<>(List.of("HmacSHA256"));
  ...
  @NotNull private Duration clockSkew = Duration.ofMinutes(1);
  ...
  public List<String> getAcceptedAlgorithms() {
    return Collections.unmodifiableList(acceptedAlgorithms);
  }

  public void setAcceptedAlgorithms(List<String> acceptedAlgorithms) {
    this.acceptedAlgorithms =
        (acceptedAlgorithms == null) ? new ArrayList<>() : new ArrayList<>(acceptedAlgorithms);
  }
