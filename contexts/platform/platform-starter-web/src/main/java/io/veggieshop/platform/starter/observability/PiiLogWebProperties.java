...
public class PiiLogWebProperties {
    ...
    /**
     * Regex patterns to redact from logs. Keep conservative, non-catastrophic patterns.
     * Examples include emails, 16-digit cards (loose), and 'password=' style assignments.
     */
    @NotNull
    private List<String> redactPatterns = new ArrayList<>(List.of(
            // emails
            "(?i)[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.[A-Z]{2,}",
            // credit-card-ish 13–19 digits (loose, word-boundaries)
            "\\b(?:\\d[ -]*?){13,19}\\b",
    ...
    public int getPayloadMaxChars() { return payloadMaxChars; }
    public void setPayloadMaxChars(int payloadMaxChars) { this.payloadMaxChars = payloadMaxChars; }

    public Set<String> getHeaderDenylist() { return headerDenylist; }
    public void setHeaderDenylist(Set<String> headerDenylist) {
        this.headerDenylist = (headerDenylist == null) ? new LinkedHashSet<>() : new LinkedHashSet<>(headerDenylist);
    }

    public List<String> getRedactPatterns() { return redactPatterns; }
    public void setRedactPatterns(List<String> redactPatterns) {
        this.redactPatterns = (redactPatterns == null) ? new ArrayList<>() : new ArrayList<>(redactPatterns);
    }
}
